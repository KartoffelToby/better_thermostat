...
def device_has_swapped_modes(self) -> bool:
    ...
    try:
        ...
        _LOGGER.error(
            f"better thermostat {self.name}: Could not load config for heat_auto_swapped"
        )
...
def mode_remap(self, hvac_mode: str, inbound: bool = False) -> str:
    ...
    _LOGGER.debug(
        f"better thermostat {self.name} {hvac_mode}: device_has_quirks: {device_has_quirks}"
    )

    if hvac_mode == HVAC_MODE_HEAT and device_has_quirks and not inbound:
        return HVAC_MODE_AUTO
    elif hvac_mode == HVAC_MODE_AUTO and device_has_quirks and inbound:
