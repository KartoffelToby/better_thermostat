...
class BetterThermostat(ClimateEntity, RestoreEntity, ABC):
    ...
    async def startup(self):
        ...
        while self.startup_running:
            ...
            self.window_open = any(self.hass.states.get(sensor).state in ("on", "open", "true") for sensor in self.window_ids)
            self.door_open = any(self.hass.states.get(sensor).state in ("on", "open", "true") for sensor in self.door_ids)
