...
class BetterThermostat(ClimateEntity, RestoreEntity, ABC):
    ...
    def __init__(
        ...
        # Initialisieren der Warteschlange für Fenstersensoren, falls definiert
        if self.window_id is not None:
            self.window_queue_task = asyncio.Queue(maxsize=1)
        
        # Initialisieren der Warteschlange für Türsensoren, falls definiert
        if self.door_id is not None:
            self.door_queue_task = asyncio.Queue(maxsize=1)
        
        # Hintergrundaufgaben erstellen
        asyncio.create_task(control_queue(self))
        
        # Hintergrundaufgabe für Fenstersteuerung erstellen
        if self.window_id is not None:
            asyncio.create_task(window_queue(self))
        
        # Hintergrundaufgabe für Türsteuerung erstellen
        if self.door_id is not None:
    ...
    def hvac_action(self):
        ...
        if self.bt_target_temp is not None and self.cur_temp is not None:
            if self.hvac_mode == HVACMode.OFF:
                ...
                and not self.door_open  # Tür darf nicht geöffnet sein
            ):
                # Heizung heizt, wenn die Zieltemperatur höher ist und weder Fenster noch Tür offen sind
                self.attr_hvac_action = HVACAction.HEATING
            elif (
                ...
                # Heizung heizt ebenfalls bei geringem Unterschied, solange Fenster und Tür geschlossen sind
                self.attr_hvac_action = HVACAction.HEATING
            else:
                # Heizung ist im Leerlauf
