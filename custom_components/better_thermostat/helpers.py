...
async def set_trv_values(self, key, value):
	...
	if key == 'temperature':
		...
		await self.hass.services.async_call(
			'number', SERVICE_SET_VALUE, {'entity_id': self.local_temperature_calibration_entity, 'value': value}, blocking=True
		)
		_LOGGER.debug("better_thermostat send %s %s", key, value)
	elif key == 'valve_position':
		await self.hass.services.async_call(
			'number', SERVICE_SET_VALUE, {'entity_id': self.valve_position_entity, 'value': value}, blocking=True
		)
		_LOGGER.debug("better_thermostat send %s %s", key, value)
