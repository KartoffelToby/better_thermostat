...
class BetterThermostat(ClimateEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        def _async_startup(*_):
            """Init on startup."""
            _LOGGER.info("Starting better_thermostat for %s with version: 1.0.0 waiting for entity to be ready...",self.name)
    ...
    async def _async_tvr_changed(self, event):
        ...
        if new_state.attributes is not None:
            ...
            #_LOGGER.debug("better_thermostat %s something changed W: %s | %s - %s | %s - %s",new_state.attributes.get('friendly_name'),self.window_open,new_state.attributes.get('system_mode'),old_state.attributes.get('system_mode'),new_state.attributes.get('current_heating_setpoint'),old_state.attributes.get('current_heating_setpoint'))
            self.async_write_ha_state()
        ...
        async with self._temp_lock:
            if None not in (
                ...
                try:
                    ...
                    _LOGGER.debug(
                        "better_thermostat triggerd States > Window open: %s Night mode: %s Mode: %s Setted: %s hasmode: %s Calibration: %s - send: %s settemp: %s curtemp: %s Model: %s Calibration type: %s Winter: %s TRV: %s",
                        self.window_open,
                        self.night_status,
                        converted_hvac_mode,
                        self._hvac_mode,
