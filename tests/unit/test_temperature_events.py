...
class TestTemperatureAcceptance:
    ...
    @pytest.mark.asyncio
    async def test_temp_change_accepted_after_interval(self, mock_bt):
    ...
    @pytest.mark.asyncio
    async def test_change_within_debounce_window_accepted(self, mock_bt):
        """Accept a change within the 5s debounce window.

        With _sig_threshold=0.0, the or-branch (_diff_q >= 0.0) is always
        True, so _interval_ok is never checked.
        """
        mock_bt.cur_temp = 20.0
    ...
    @pytest.mark.asyncio
    async def test_homematicip_change_within_600s_accepted(self, mock_bt):
        """Accept a HomematicIP change within the 600s debounce window.

        The zero threshold bypasses the HomematicIP debounce the same way.
        """
    ...
    @pytest.mark.asyncio
    async def test_identical_temp_accepted(self, mock_bt):
        """Accept an identical temperature (_diff=0.0 >= threshold 0.0)."""
        mock_bt.cur_temp = 20.0
        mock_bt.last_external_sensor_change = datetime.now() - timedelta(seconds=60)
...
class TestAccumulationTracking:
    ...
    @pytest.mark.asyncio
    async def test_accum_delta_reset_after_accept(self, mock_bt):
        """Reset accum_delta to 0 after the temperature is accepted."""
        mock_bt.cur_temp = 20.0
    ...
    async def test_accum_resets_on_direction_flip(self, mock_bt):
        """Reset accum_delta and accum_dir after a direction change and accept."""
        mock_bt.cur_temp = 20.0
        mock_bt.accum_delta = 0.1
        mock_bt.accum_dir = 1

        event = _make_event(State(SENSOR_ID, "19.9"))
        await trigger_temperature_change(mock_bt, event)
    ...
    @pytest.mark.asyncio
    async def test_pending_temp_cleared_after_accept(self, mock_bt):
        """Clear pending_temp after the temperature is accepted."""
        mock_bt.cur_temp = 20.0
        event = _make_event(State(SENSOR_ID, "20.1"))
...
class TestPlateauLogic:
    ...
    @pytest.mark.asyncio
    async def test_plateau_timer_not_started_with_zero_threshold(self, mock_bt):
        """Do not start a plateau timer because _is_significant is always True.

        The `not _is_significant` guard (line 466) is never True when
        _sig_threshold=0.0, so the plateau scheduling block is unreachable.
        """
        mock_bt.cur_temp = 20.0
    ...
    @pytest.mark.asyncio
    async def test_no_plateau_timer_for_small_change(self, mock_bt):
        """Do not schedule a plateau timer for a small temperature change."""
        mock_bt.cur_temp = 20.0
    ...
    @pytest.mark.asyncio
    async def test_accept_reason_is_always_significant(self, mock_bt):
        """Always accept with reason 'significant', never 'accumulated'."""
        mock_bt.cur_temp = 20.0
        mock_bt.accum_delta = 1.0
        mock_bt.accum_dir = 1
